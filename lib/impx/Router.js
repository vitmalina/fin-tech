import { w2base } from '/lib/w2ui/w2ui-2.0.es6.js'

export class Router extends w2base {
    constructor() {
        super()
        this.routes = {}
        this.routeRE = {}
    ...
    add(route, handler) {
        ...
        if (typeof this.trigger == 'function') {
            edata = this.trigger('add', { target: 'self', route: route, handler: handler })
            if (edata.isCancelled === true) return false
        }
        // default behavior
        this.routes[route] = handler
        // if events are available
        if (typeof this.trigger == 'function') edata.finish();
        return this;
    }
    ...
    remove(route) {
        ...
        // if events are available
        if (typeof this.trigger == 'function') {
            edata = this.trigger('remove', { target: 'self', route: route })
            if (edata.isCancelled === true) return false
        }
        // default behavior
    ...
    process() {
        ...
        Object.keys(this.routeRE).forEach(r => {
            ...
            if (tmp != null) { // match
                ...
                // if events are available
                if (typeof this.trigger == 'function') {
                    edata = this.trigger('route', { target: 'self', route: r, params: params })
                    if (edata.isCancelled === true) return false
                }
                // default handler
        ...
        if (!isExact) {
            Object.keys(this.baseRoutes).forEach(route => {
                let obj = this
                let mod = { route: route, path: this.baseRoutes[route] }
                let rt = mod.route
                if (rt != null) {
                ...
                function checkRoute(str) {
                    ...
                    if (!mod.ready && str && obj.routeRE[str].path.exec(hash)) {
                        ...
                        if (!isLoaded) {
                            ...
                            child.onload = (event) => { obj.go(obj.get()) }
                            child.onerror = (event) => {
                                let edata = obj.trigger('error', { target: 'self', hash: hash, originalEvent: event, 404: true })
                                if (edata.isCancelled === true) return false
                                // if events are available
                                edata.finish()
                            }
                            if (obj.verbose) console.log(`ROUTER: Auto Load Module "${mod.path}" for path "${mod.route}"`)
        ...
        if (!isAutoLoad && !isFound) {
            let edata = this.trigger('error', { target: 'self', hash: hash })
            if (edata.isCancelled === true) return false
            // if events are available
            edata.finish()
        }
        if (!isAutoLoad && this.verbose) {
            // path not found
            if (!isExact) {
